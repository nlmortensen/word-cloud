...
def calculate_frequencies(file_contents):
    ...
    wordcloud_list = []
    frequencies = {}
    #check each character in the word to see if it's a punctuation
    for char in file_contents:
        if char not in punctuations:
            #create new word without punctuation
            words += char
    
    file_words = words.split()
    
    #check if each word is alphabetic and if it's an uninteresting word
    for word in file_words:
        word = word.lower()
        if word.isalpha() == True and word not in uninteresting_words:
                wordcloud_list.append(word)
    
    #add words and their frequencies to the dictionary
